...
export function setup( opts?, options?, customCss?, customfavIcon?, swaggerUrl?, customSiteTitle?) {
...
export var serve = [swaggerInitFn, swaggerAssetMiddleware()]
...
/**
 * @author <Mayur Tikundi> mayurtikundi12@gmail.com
 */
...
const swaggerFilePath = __dirname + "/swagger.json";

 function GetSwaggerDoc(){
    let  swaggerDocument = null;
    if (existsSync(swaggerFilePath)) {
        const raw_swagger_data = readFileSync(swaggerFilePath);
        swaggerDocument = JSON.parse(raw_swagger_data.toString());
    }else{
        writeFileSync(swaggerFilePath,JSON.stringify(swagger_base));
